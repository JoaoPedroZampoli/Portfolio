  ...
  return (
    <section className="min-h-screen flex flex-col">
      ...
      <div className="min-h-screen flex items-center justify-center relative overflow-hidden">
        ...
        <motion.div className="w-full max-w-7xl mx-auto px-6 flex flex-col lg:flex-row items-center gap-16 relative z-10"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  ...
                  <p className="text-lg text-default-600 leading-relaxed">
                    Mini-game 2D criado em Unity inspirado em clássicos de plataforma. 
                    O jogador controla um gato em busca de seu sachê, navegando por 
                    diferentes níveis com mecânicas de pulo e coleta.
                  </p>
                </div>
                
                <div className="flex flex-wrap gap-2">
                  <span className="px-3 py-1 text-xs font-medium rounded-full bg-gray-100 text-gray-700">Unity</span>
                  <span className="px-3 py-1 text-xs font-medium rounded-full bg-purple-100 text-purple-700">C#</span>
                  <span className="px-3 py-1 text-xs font-medium rounded-full bg-yellow-100 text-yellow-700">Game Design</span>
                </div>
                    ...
                    {projeto1Imagens.map((_, index) => (
                      <button 
                        ...
                        className={`w-2 h-2 rounded-full transition-all ${
                          selectedImg1Index === index ? 'bg-white scale-125' : 'bg-white/50'
                        }`}
                      />
                    ))}
                  </div>
                  ...
                  {projeto1Imagens.map((imgSrc, index) => (
                    <button 
                      ...
                      className={`flex-shrink-0 w-20 h-12 rounded-lg overflow-hidden border-2 transition-all ${
                        selectedImg1Index === index ? 'border-blue-500 scale-105' : 'border-transparent opacity-70 hover:opacity-100'
                      }`}
                      onClick={() => setSelectedImg1Index(index)}
                    >
                      <Image 
            ...
            <motion.div 
                  ...
                  <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-2">
                    {projeto2Imagens.map((_, index) => (
                      <button 
                        ...
                        className={`w-2 h-2 rounded-full transition-all ${
                          selectedImg2Index === index ? 'bg-white scale-125' : 'bg-white/50'
                        }`}
                      />
                    ))}
                  </div>
                ...
                >
                  {projeto2Imagens.map((imgSrc, index) => (
                    <button 
                      key={index}
                      className={`flex-shrink-0 w-20 h-12 rounded-lg overflow-hidden border-2 transition-all ${
                        selectedImg2Index === index ? 'border-green-500 scale-105' : 'border-transparent opacity-70 hover:opacity-100'
                      }`}
                      onClick={() => setSelectedImg2Index(index)}
                    >
                      <Image 
              ...
              <div className="space-y-6 lg:order-1">
                <div className="space-y-4">
                  ...
                  <p className="text-lg text-default-600 leading-relaxed">
                    Plataforma que visa melhorar a acessibilidade em transportes públicos. 
                    O sistema mapeia níveis de deficiência e sugere adequações necessárias 
                    para melhor utilização do transporte metropolitano.
                  </p>
                </div>
                
                <div className="flex flex-wrap gap-2">
                  <span className="px-3 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-700">React</span>
                  <span className="px-3 py-1 text-xs font-medium rounded-full bg-green-100 text-green-700">Node.js</span>
                  <span className="px-3 py-1 text-xs font-medium rounded-full bg-orange-100 text-orange-700">PostgreSQL</span>
                </div>
